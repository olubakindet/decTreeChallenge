...
### Answer 1: Numerical vs Categorical Encoding

Zip code should be modeled categorically, not numerically, because zip codes are non-ordinal labels. A house in zip code 50013 is not inherently "more" than a house in 50012 in the same way a larger lot area is more than a smaller one. When zip code is left as a number, the tree is forced to create threshold rules such as `zipCode > 50012.5`. That split has arithmetic form but no business meaning. It treats neighborhood identity as if it were a continuous scale, which can suppress true neighborhood effects or misstate them. In this analysis, the numerical encoding path made zip code look weak in feature importance, even though neighborhood should matter for price formation. One-hot encoding is a practical correction in scikit-learn because it converts each zip code into a direct membership question: is the home in this neighborhood or not. That maps to how buyers and sellers actually think about location. For this problem, zip code is a non-ordinal categorical variable and should always be modeled as categorical.

### Answer 2: R vs Python Implementation Differences

R and Python differ because `rpart` can split categorical variables directly, while scikit-learn trees cannot. For unordered categorical predictors, CART-style splitting can evaluate binary partitions of category levels, so one split can represent a grouped neighborhood test instead of a numeric threshold. In scikit-learn, the model cannot do this natively, and the official documentation at `https://scikit-learn.org/stable/modules/tree.html` states: "Able to handle both numerical and categorical data. However, the scikit-learn implementation does not support categorical variables for now." That limitation pushes Python users toward one-hot encoding, which breaks a single zip code feature into several binary columns. The tree then evaluates each indicator independently, so interpretation shifts from one coherent neighborhood variable to many separate flags. This often makes feature importance less clean for business communication because "zipCode" is no longer a single ranked driver. For this specific task, R gives a more natural representation of categorical splits, while Python requires preprocessing workarounds that can reduce interpretability.

### Answer 3: Python Alternatives with Better Categorical Handling

A strong Python alternative is LightGBM, which supports categorical features without forcing one-hot encoding as the default path. LightGBM documentation states: "LightGBM offers good accuracy with integer-encoded categorical features. LightGBM applies Fisher (1958) to find the optimal split over categories as described here. This often performs better than one-hot encoding." LightGBM is a gradient boosting framework rather than a single standalone decision-tree package, but it is still highly relevant because each tree in the ensemble uses native categorical split logic. This matters because it lets the model treat category structure as category structure, rather than as a set of unrelated dummy columns. In practice, that can produce cleaner split logic and more faithful importance patterns when business variables are truly nominal, like zip code, product type, or channel labels. Source URL: [https://lightgbm.readthedocs.io/en/latest/Advanced-Topics.html#categorical-feature-support](https://lightgbm.readthedocs.io/en/latest/Advanced-Topics.html#categorical-feature-support).
